...
def find_open_invoices_for_contact(contact_name):
    ...
    OPEN_STATUSES = {"לא שולם", "שולם חלקית", "unpaid", "partially paid", "partially_paid", "sent", "overdue", "draft", ""}
    result = [i for i in invoices if (i.get("Status") or "").lower() in OPEN_STATUSES or (i.get("Status") or "") in OPEN_STATUSES or i.get("Status") is None]
    ...
    if pending == "customer_status_nav":
        ...
        if _pay_nav:
            ...
            try:
                ...
                return ""
        # חשבונית חדשה מעמוד לקוח - זיהוי מילות מוצר
